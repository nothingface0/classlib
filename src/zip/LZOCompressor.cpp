...
#define STATIC_CHECK(x) do { switch (0) { case 0: case (x): ; } } while (0)

/// Maximum possible data expansion during compression, as specified in the LZO FAQ.
#define lzo_worst_compress(algo, x) (((algo) < LZO2A) \
				     ? ((x) + ((x) / 16) + 64 + 3) \
				     : ((x) + ((x) / 8) + 128 + 3))
...
#define LZO_DCALL(rc, func, in, inlen, out, outlen, workmem, level) \
  if ((rc = func (in, inlen, out, outlen, workmem, 0, 0, 0, level)) != LZO_E_OK) \
    throwLZOError(#func, rc);
...
#define LZO_OPTCALL(opt, rc, func, out, outlen, inlen)                 \
  ...
  {                                                                    \
    ...
    if (m_optbuf.size() < newlen) m_optbuf.resize (newlen, '\0');      \
    if ((rc = func(out, outlen, (lzo_bytep) &m_optbuf[0], &newlen, 0)) \
        != LZO_E_OK || newlen != inlen)                                \
      throwLZOError(#func, rc);                                        \
  }
...
void
LZOCompressor::level (unsigned level)
{
    m_level = level;
}
...
unsigned
LZOCompressor::level (void) const
{
    return m_level;
}
...
/** Set current optimisation mode.
    ...
    change will take effect the next time something is compressed.  */
void
LZOCompressor::optimise (bool value)
{
    m_optimise = value;
}
...
bool
LZOCompressor::optimise (void) const
{
    return m_optimise;
}
...
void
LZOCompressor::input (const void *buffer, IOSize length)
{
...
bool
LZOCompressor::finished (void) const
{
    return m_len == 0 && m_buffered.empty ();
}
...
bool
LZOCompressor::flushed (void) const
{
    return m_len == 0 && m_buffered.empty ();
}
...
IOSize
LZOCompressor::compress (void *into, IOSize length)
{
    union lzo_workmem
    {
    ...
    {
    case LZO1 << 24 | 1 << 8:
        LZO_SCALL(rc, lzo1_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;

    case LZO1 << 24 | 99 << 8:
        LZO_SCALL(rc, lzo1_99_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1A << 24 | 99 << 8:
        LZO_SCALL(rc, lzo1a_99_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1B << 24 | 8 << 8:
    case LZO1B << 24 | 9 << 8:
        LZO_LCALL(rc, lzo1b_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem, (m_level >> 8) & 0xf);
        break;

    case LZO1B << 24 | 99 << 8:
        LZO_SCALL(rc, lzo1b_99_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1B << 24 | 999 << 8:
        LZO_SCALL(rc, lzo1b_999_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1C << 24 | 99 << 8:
        LZO_SCALL(rc, lzo1c_99_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1C << 24 | 999 << 8:
        LZO_SCALL(rc, lzo1c_999_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1F << 24 | 999 << 8:
        LZO_SCALL(rc, lzo1f_999_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1X << 24 | 1 << 8 | 11:
        LZO_SCALL(rc, lzo1x_1_11_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1X << 24 | 1 << 8 | 12:
        LZO_SCALL(rc, lzo1x_1_12_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1X << 24 | 1 << 8 | 15:
        LZO_SCALL(rc, lzo1x_1_15_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1X << 24 | 999 << 8 | 9:
        LZO_DCALL(rc, lzo1x_999_compress_level,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem, m_level & 0xf);
	LZO_OPTCALL(m_optimise, rc, lzo1x_optimize,
		    (lzo_bytep) dest, destlen, m_len);
        break;

    case LZO1Y << 24 | 1 << 8:
        LZO_SCALL(rc, lzo1y_1_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
    ...
    case LZO1Y << 24 | 999 << 8 | 9:
        LZO_DCALL(rc, lzo1y_999_compress_level,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem, m_level & 0xf);
	LZO_OPTCALL(m_optimise, rc, lzo1y_optimize,
		    (lzo_bytep) dest, destlen, m_len);
        break;
    ...
    case LZO1Z << 24 | 999 << 8 | 9:
        LZO_DCALL(rc, lzo1z_999_compress_level,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem, m_level & 0xf);
	break;
    ...
    case LZO2A << 24 | 999 << 8:
        LZO_SCALL(rc, lzo2a_999_compress,
		  (lzo_bytep) m_input, m_len,
		  (lzo_bytep) dest, &destlen,
		  &workmem);
        break;
...
IOSize
LZOCompressor::estimateWorstSize (void) const
{
