...
template <typename T>
inline typename define_if<(sizeof((time_t (*)(tm *)) &timegm) > 0), time_t>::type
...
tm_get_gmtoff_hasit(Time::ValueType n, int *isdst)
{
    T localtm = Time(n).local();
    if (isdst) *isdst = localtm.tm_isdst;
    return localtm.tm_gmtoff * SEC_NSECS;
}

template <typename T>
inline typename define_if<have_struct_field<T, long, &T::tm_gmtoff>::value, Time::ValueType>::type
tm_get_gmtoff(Time::ValueType n, int *isdst, void *)
{ return tm_get_gmtoff_hasit<T>(n, isdst); }

template <typename T>
inline typename define_if<have_struct_field<T, int, &T::tm_gmtoff>::value, Time::ValueType>::type
tm_get_gmtoff(Time::ValueType n, int *isdst, void *)
{ return tm_get_gmtoff_hasit<T>(n, isdst); }

template <typename T>
inline Time::ValueType
tm_get_gmtoff(Time::ValueType n, int *isdst, ...)
{
    // Adapted from WINE.
...
// to deduce whether a particular data member exists or not.
template <typename T>
inline typename define_if<have_struct_field<T, const char *, &T::tm_zone>::value, const char *>::type
tm_get_zone(T &local, void *)
{ return local.tm_zone; }

template <typename T>
inline typename define_if<have_struct_field<T, char *, &T::tm_zone>::value, const char *>::type
tm_get_zone(T &local, void *)
{ return local.tm_zone; }
