...
extern void timegm(lat::this_overload_does_not_exist *);
...
// Utility to detect and use timegm() function if it is available, and fall
// back on private replacement code if it isn't.
template <typename T>
inline
    typename define_if<(sizeof((time_t(*)(tm *)) & timegm) > 0), time_t>::type
    call_timegm(T *t, void *) {
  return timegm(t);
}

template <typename T> inline time_t call_timegm(T *t, ...) {
...
// Utilities to calculate determine GMT offset from struct tm, depending
// on whether struct tm contains tm_gmtoff member or not.  Uses templates
// to deduce whether a particular data member exists or not.
template <typename T>
inline Time::ValueType tm_get_gmtoff_hasit(Time::ValueType n, int *isdst) {
  T localtm = Time(n).local();
  if (isdst)
    *isdst = localtm.tm_isdst;
  return localtm.tm_gmtoff * SEC_NSECS;
}

template <typename T>
inline typename define_if<have_struct_field<T, long, &T::tm_gmtoff>::value,
                          Time::ValueType>::type
tm_get_gmtoff(Time::ValueType n, int *isdst, void *) {
  return tm_get_gmtoff_hasit<T>(n, isdst);
}

template <typename T>
inline typename define_if<have_struct_field<T, int, &T::tm_gmtoff>::value,
                          Time::ValueType>::type
tm_get_gmtoff(Time::ValueType n, int *isdst, void *) {
  return tm_get_gmtoff_hasit<T>(n, isdst);
}
...
template <typename T>
inline Time::ValueType tm_get_gmtoff(Time::ValueType n, int *isdst, ...) {
  // Adapted from WINE.
  time_t utctime = n / SEC_NSECS;
  T localtm = *localtime(&utctime);
  int savedaylight = localtm.tm_isdst;
  T gmt = *gmtime(&utctime);

  gmt.tm_isdst = savedaylight;
  if (isdst)
    *isdst = savedaylight;

  return (utctime - mktime(&gmt)) * SEC_NSECS;
}

// Utilities to calculate determine time zone from struct tm, depending
// on whether struct tm contains tm_zone member or not.  Uses templates
// to deduce whether a particular data member exists or not.
template <typename T>
inline
    typename define_if<have_struct_field<T, const char *, &T::tm_zone>::value,
                       const char *>::type
    tm_get_zone(T &local, void *) {
  return local.tm_zone;
}

template <typename T>
inline typename define_if<have_struct_field<T, char *, &T::tm_zone>::value,
                          const char *>::type
tm_get_zone(T &local, void *) {
  return local.tm_zone;
}

template <typename T> inline const char *tm_get_zone(T &local, ...) {
...
Time Time::build(bool local, const tm &base, TimeSpan diff /* = 0 */) {
  tm tmp(base);
  if (local)
    return Time(mktime(&tmp), 0) + diff;
  else
    return Time(call_timegm<struct tm>(&tmp, 0), 0) + diff;
}
...
tm Time::split(bool local, long *nsecpart /* = 0 */) const {
  if (nsecpart)
...
tm Time::utc(long *nsecpart /* = 0 */) const { return split(false, nsecpart); }
...
tm Time::local(long *nsecpart /* = 0 */) const { return split(true, nsecpart); }
...
long Time::nsecond(void) const { return m_nsecs % SEC_NSECS; }
...
Time::ValueType Time::utcoffset(int *daylight /* = 0 */) const {
  return tm_get_gmtoff<struct tm>(m_nsecs, daylight, 0);
}
...
const char *Time::timezone(int *daylight /* = 0 */) const {
  ...
  return tm_get_zone<struct tm>(localtm, 0);
}
