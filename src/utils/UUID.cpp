...
int UUID::compare(const Info &a, const Info &b) {
...
/** Format the UUID into the buffer @a buf, which must have space at
    ...
    null.  */
void UUID::format(char *buf) const {
  ASSERT(DebugAids::validate(buf, 37, true));
  Info info;
  unpack(m_uuid, info);
  sprintf(buf, "%08x-%04x-%04x-%04x-%02x%02x%02x%02x%02x%02x", info.timeLow,
          info.timeMid, info.timeHiVer, info.clockSeq, info.node[0],
          info.node[1], info.node[2], info.node[3], info.node[4], info.node[5]);
}

/** Construct a null UUID.  */
UUID::UUID(void) { memset(m_uuid, 0, sizeof(m_uuid)); }
...
UUID::UUID(const char *text) { parse(text, strlen(text), m_uuid); }
...
UUID::UUID(Binary raw) { memcpy(m_uuid, raw, sizeof(m_uuid)); }

/** Copy UUID @a x.  */
UUID::UUID(const UUID &x) { memcpy(m_uuid, x.m_uuid, sizeof(m_uuid)); }
...
int UUID::type(void) const { return (m_uuid[6] >> 4) & 0xf; }

/** Return the variant component of the UUID.  */
int UUID::variant(void) const {
  if ((m_uuid[8] & 0x80) == 0)
    return VARIANT_NCS;
  if ((m_uuid[8] & 0x40) == 0)
    return VARIANT_DCE;
...
void UUID::unpack(const Binary from, Info &to) {
...
void UUID::pack(const Info &from, Binary to) {
...
void UUID::parse(const char *text, size_t len, Binary to) {
  ...
  __extension__ // shut up non-ansi format warnings
      ...
      sscanf(text + 24,
             ...
             &info.node[0], &info.node[1], &info.node[2], &info.node[3],
             &info.node[4], &info.node[5]);
...
void UUID::getRandomData(void *buf, size_t len) {
...
/** Fill the ethernet hardware address of this system into @a id
    ...
    address was found. */
bool UUID::getNodeID(unsigned char *id) {
