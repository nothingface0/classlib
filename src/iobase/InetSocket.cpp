...
static void initInetAddr(sockaddr_storage &address, socklen_t &addrlen,
...
InetAddress::InetAddress(unsigned short port /* = 0 */) {
  ...
  if (port != 0) {
    ...
    initInetAddr(m_address, m_addrlen, 0, srvnr,
                 AI_PASSIVE | AI_NUMERICSERV | AI_ADDRCONFIG, AF_UNSPEC,
                 SOCK_STREAM);
  }
}
...
InetAddress::InetAddress(unsigned long address, const char *service,
  ...
  if (!p)
    throw NetworkError("getprotobyname()", proto);
...
InetAddress::InetAddress(const char *host, unsigned short port /* = 0 */,
  ...
  char srvnr[32];
  sprintf(srvnr, "%d", port);
  initInetAddr(m_address, m_addrlen, host, srvnr, AI_NUMERICSERV | flags,
               family, SOCK_STREAM);
}

/** Create an internet domain address to a named host using a
    named service using the numeric protocol.  */
InetAddress::InetAddress(const char *host, const char *service,
                         int proto /* = SOCK_STREAM */,
                         int family /* = AF_UNSPEC */,
                         int flags /* = AI_ADDRCONFIG */) {
  initInetAddr(m_address, m_addrlen, host, service, flags, family, proto);
}

/** Create an internet domain address to a named host using a
    named service using the named protocol.  */
InetAddress::InetAddress(const char *host, const char *service,
                         const char *proto, int family /* = AF_UNSPEC */,
                         int flags /* = AI_ADDRCONFIG */) {
  protoent *p = getprotobyname(proto);
  if (!p)
    throw NetworkError("getprotobyname()", proto);
  initInetAddr(m_address, m_addrlen, host, service, flags, family, p->p_proto);
}

/** Copy an internet domain address.  */
InetAddress::InetAddress(const InetAddress &a) : SocketAddress(a) {}

/** Copy an internet domain address. */
InetAddress &InetAddress::operator=(const InetAddress &a) {
  SocketAddress::operator=(a);
  return *this;
}
...
std::string InetAddress::service(int flags /* = 0 */) const {
  char name[1024];
  int err = getnameinfo((sockaddr *)&m_address, m_addrlen, 0, 0, name,
                        sizeof(name), flags);
  if (err == EAI_NONAME)
...
/** Get the host name designated by the address.  Returns an empty
    string if the name cannot be resolved.  */
std::string InetAddress::hostname(int flags /* = 0 */) const {
  char name[1024];
  int err = getnameinfo((sockaddr *)&m_address, m_addrlen, name, sizeof(name),
                        0, 0, flags);
  if (err == EAI_NONAME && m_address.ss_family == AF_INET6 &&
      !memcmp(&((sockaddr_in6 *)&m_address)->sin6_addr, &in6addr_any,
              sizeof(in6_addr)))
    return "::";
  else if (err == EAI_NONAME && m_address.ss_family == AF_INET &&
...
InetSocket::InetSocket(const InetAddress &to, int type /* = SOCK_STREAM */,
                       int protocol /* = 0 */) {
  Socket::create(to.family(), type, protocol);
  Socket::connect(to);
}
...
/// @reimp
void InetSocket::create(int family /* = AF_UNSPEC */,
                        int type /* = SOCK_STREAM */, int protocol /* = 0 */) {
  Socket::create(family, type, protocol);
}

/// @reimp
void InetSocket::bind(const InetAddress &a) { Socket::bind(a); }

/// @reimp
void InetSocket::connect(const InetAddress &a) { Socket::connect(a); }

/** Retrieve the local address of the socket.  */
InetAddress InetSocket::sockname(void) {
  InetAddress addr;
...
InetAddress InetSocket::peername(void) {
  InetAddress addr;
...
/// @reimp
IOSize InetSocket::read(void *into, IOSize n, InetAddress &source) {
  return Socket::read(into, n, source);
}

/// @reimp
IOSize InetSocket::read(IOBuffer into, InetAddress &source) {
  return read(into.data(), into.size(), source);
}

/// @reimp
IOSize InetSocket::peek(void *into, IOSize n, InetAddress &source) {
  return Socket::peek(into, n, source);
}

/// @reimp
IOSize InetSocket::peek(IOBuffer into, InetAddress &source) {
  return peek(into.data(), into.size(), source);
}

/// @reimp
IOSize InetSocket::write(const void *from, IOSize n, const InetAddress &dest) {
  return Socket::write(from, n, dest);
}

/// @reimp
IOSize InetSocket::write(IOBuffer from, const InetAddress &dest) {
  return write(from.data(), from.size(), dest);
}

/// @reimp
Socket *InetSocket::copy(void) { return new InetSocket(*this); }
